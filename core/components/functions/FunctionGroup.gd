...
signal on_trigger_added(component: EngineComponent, up_method: String, down_method: String, name: String, row: int, column: int)
...
func add_trigger(p_component: EngineComponent, p_up_method: String, p_down_method: String, p_name: String, p_row: int, p_column: int, no_signal: bool = false) -> bool:
	...
	if not no_signal:
		on_trigger_added.emit(p_component, p_up_method, p_down_method, p_name)
	...
	for row: Variant in triggers:
		...
		for column: Variant in triggers[row]:
			...
			ComponentDB.request_component(triggers[row][column].component, func (component: EngineComponent):
				add_trigger(component, triggers[row][column].up, triggers[row][column].down, triggers[row][column].name, row, column, true)
			)
