...
func _init() -> void:
	...
	settings_manager.set_inheritance_array(["Debug"])
	settings_manager.register_networked_methods_auto([
		reset,
		quit,
		crash,
		echo,
		dump_networked_objects,
		dump_fixture_data,
	])

	Utils.ensure_folder_exists(debug_file_location)
...
## Returns the input
func echo(p_echo_text: String) -> String:
	return p_echo_text
