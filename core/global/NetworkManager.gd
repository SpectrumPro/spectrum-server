...
func send_message(p_command: Variant, p_node_filter: NetworkSession.NodeFilter = NetworkSession.NodeFilter.AUTO) -> Error:
	return _active_handlers["Constellation"].send_command(objects_to_uuids(p_command), p_node_filter)
		...
		TYPE_DICTIONARY:
			if "_object_ref" in p_data.keys():
				if _registered_network_objects.has_left(p_data._object_ref):
					...
					if ClassList.has_class(type_convert(["_class_name"], TYPE_STRING)):
...
func _on_command_recieved(p_from: NetworkNode, p_type: Variant.Type, p_command: Variant) -> void:
	if p_command is Dictionary:
		...
		var args: Array = uuids_to_objects(type_convert(str_to_var(type_convert(p_command.get("args", "[]"), TYPE_STRING)), TYPE_ARRAY))
