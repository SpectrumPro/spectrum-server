...
## The MTU for tcp payloads, if this is exceeded the command will be sent as a multipart message
const TCP_MTP: int = 65000
...
func send_message_udp(p_message: ConstaNetHeadder) -> Error:
	...
	if buffer.size() > UDP_MTP:
		_network._logv(ConstaNetHeadder.Type.keys()[p_message.type], " is too large to send as a single UDP frame (", buffer.size(), ") Sending as multipart")
...
func send_message_tcp(p_message: ConstaNetHeadder) -> Error:
	...
	if buffer.size() > TCP_MTP:
		_network._logv(ConstaNetHeadder.Type.keys()[p_message.type], " is too large to send as a single TCP frame (", buffer.size(), ") Sending as multipart")
		...
		multi_part.multi_part_id = UUID_Util.v4()
		multi_part.num_of_chunks = int(ceil(buffer.size() / float(TCP_MTP)))
		
		while offset < buffer.size():
			var current_size: int = min(TCP_MTP, buffer.size() - offset)
			var payload: PackedByteArray = buffer.slice(offset, offset + current_size)
			
			multi_part.chunk_id = chunk_id
			multi_part.data = payload
			
			var error: Error = _tcp_socket.put_data(multi_part.get_as_packet())
		...
		var errcode: Error = _tcp_socket.put_data(buffer)
		return errcode
